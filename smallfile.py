...
class MFSyncExc(Exception):
    def __init__(self, oserror_in):
        self.os_error = oserror_in
        return
    def __str__(self):
        return "error syncing file system after file accesses complete: " + os.strerror(self.os_error)
...
class smf_invocation:
    ...
    def __init__(self):
        ...
        self.reset()
        self.invocation_count += 1    # FIXME: not thread-safe
        self.invocation_id = self.invocation_count # used to generate unique write/read contents per file
    ...
    def clone(smf_instance):
        ...
        new.log = None
        new.tid = None
        new.invocation_id = s.invocation_count
        s.invocation_count += 1  # FIXME: not thread-safe
        new.reset()
        return new
    ...
    def prepare_buf(self):
        ...
        unique_offset = self.invocation_id * self.filenum
            ...
            while len(dpath) > len(tree) + 1:
                if not exists(dpath):
                        ...
                        try:
                          ...
                          print 'error deleting directory dpath: %s'%e
    ...
    def do_symlink(self):
        while self.do_another_file():
            fn = self.mk_file_nm(self.src_dir)
            fn2 = self.mk_file_nm(self.dest_dir) + '.s'
            self.op_starttime()
            os.symlink(fn, fn2)
            self.op_endtime(self.opname)
        ...
        while self.do_another_file():
            ...
            try:
              ...
              while remaining_kb > 0:
                ...
                if self.verify_read:
                  #print 'iid=%d filenum=%d  buf: %s'%(self.invocation_id, self.filenum, self.buf)
                  #print 'iid=%d filenum=%d read: %s'%(self.invocation_id, self.filenum, bytesread)
    ...
    def do_rename(self):
        in_same_dir = (self.dest_dir == self.src_dir)
        while self.do_another_file():
            fn1 = self.mk_file_nm(self.src_dir)
            fn2 = self.mk_file_nm(self.dest_dir)
            if in_same_dir:
    ...
    def do_delete_renamed(self):
        in_same_dir = (self.dest_dir == self.src_dir)
        while self.do_another_file():
            fn = self.mk_file_nm(self.dest_dir)
            if in_same_dir:
    ...
    def do_cleanup(self):
        ...
        while self.do_another_file():
            sym = self.mk_file_nm(self.dest_dir) + '.s'
            ensure_deleted(sym)
            basenm = self.mk_file_nm(self.src_dir)
            fn = basenm
            ensure_deleted(fn)
            fn = self.mk_file_nm(self.dest_dir)
            ensure_deleted(fn)
    ...
    def do_workload(self):
        ...
        try:
            ...
            if o == "create":
                ...
                if xattr_not_installed:
                  raise Exception('extended attribute operation requested but xattr module not installed')
                self.do_getxattr()
            elif o == "setxattr":
                if xattr_not_installed:
                  raise Exception('extended attribute operation requested but xattr module not installed')
...
class Test(unittest.TestCase):
    ...
    def mk_files(self):
        self.cleanup_files()
        self.runTest("create")
        self.assertTrue(exists(self.invok.mk_file_nm(self.invok.src_dir)))
        assert (os.path.getsize(self.invok.mk_file_nm(self.invok.src_dir)) == self.invok.total_sz_kb * 1024)

    def test1_recreate_src_dest_dirs(self):
        self.deltree(self.invok.src_dir)
        os.mkdir(self.invok.src_dir)
        self.deltree(self.invok.dest_dir)
    ...
    def test_c3_Symlink(self):
        ...
        self.assertTrue(exists(self.invok.mk_file_nm(self.invok.dest_dir)+'.s'))
    ...
    def test_e_Rename(self):
        ...
        self.mk_files()
        self.runTest("rename")
        fn = self.invok.mk_file_nm(self.invok.dest_dir)
